...
namespace mpilab::domain
{
     ...
     *
     * @tparam Value 中文：待检查的返回值类型。 English: Return value type to inspect.
     * @note 中文：这里接受可转换为 double 的算术类型，避免把内存布局或具体标量存储策略绑进领域接口。 English: This accepts arithmetic types convertible to double so the domain interface does not bind memory layout or concrete scalar storage policy.
     */
     ...
     *
     * @tparam Matrix 中文：待检查的矩阵类型。 English: Matrix type to inspect.
     * @note 中文：构造参数表示逻辑二维范围，不表示任何内存布局；该概念故意不要求 rows()、cols()、迭代器或连续内存。 English: The constructor parameters represent the logical two-dimensional extent, not any memory layout; this concept intentionally does not require rows(), cols(), iterators, or contiguous memory.
     */
