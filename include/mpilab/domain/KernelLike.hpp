...
namespace mpilab::domain
{
     ...
     *
     * @tparam Kernel 待检查的核类型。 / Kernel type to inspect.
     * @tparam Matrix 核操作的矩阵映射类型。 / Matrix mapping type operated on by the kernel.
     * @note C++ concept 无法形式化证明无副作用（side effect）或引用透明性（referential transparency）；这里用空类型和 const 调用约束表达可由类型系统检查的“无状态纯函数”部分。 / A C++ concept cannot formally prove absence of side effects or referential transparency; this encodes the stateless part that the type system can check through an empty type and const invocation.
     */
