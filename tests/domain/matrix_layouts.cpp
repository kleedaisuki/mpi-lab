 ...
 *
 * @tparam Matrix 中文：矩阵类型。 English: Matrix type.
 * @param matrix 中文：待填充矩阵。 English: Matrix to populate.
 */
template <typename Matrix>
void fill_matrix(Matrix& matrix)
{
    /**
     * @brief 中文：逻辑高度缓存。 English: Cached logical height.
     */
    const std::size_t height = matrix.height();

    /**
     * @brief 中文：逻辑宽度缓存。 English: Cached logical width.
     */
 ...
 *
 * @tparam Matrix 中文：矩阵类型。 English: Matrix type.
 * @param matrix 中文：待验证矩阵。 English: Matrix to validate.
 */
template <typename Matrix>
...
{
    /**
     * @brief 中文：行主序测试矩阵。 English: Row-major test matrix.
     */
    mpilab::domain::RowMajorMatrix<> row_major(3, 2);

    /**
     * @brief 中文：列主序测试矩阵。 English: Column-major test matrix.
     */
    mpilab::domain::ColumnMajorMatrix<> column_major(3, 2);

    /**
     * @brief 中文：带跨距测试矩阵。 English: Strided test matrix.
     */
    mpilab::domain::StridedRowMajorMatrix<> strided(3, 2, 5);

    /**
     * @brief 中文：逐行分配测试矩阵。 English: Jagged per-row test matrix.
     */
    mpilab::domain::JaggedRowMajorMatrix<> jagged(3, 2);

    /**
     * @brief 中文：分块测试矩阵。 English: Blocked-layout test matrix.
     */
    mpilab::domain::BlockedRowMajorMatrix<> blocked(4, 4, 2, 2);

    /**
     * @brief 中文：Morton 测试矩阵。 English: Morton-layout test matrix.
     */
    ...
    /**
     * @brief 中文：期望的行主序底层存储。 English: Expected row-major backing storage.
     */
    const std::vector<double> expected_row_major{0.0, 1.0, 2.0, 100.0, 101.0, 102.0};

    /**
     * @brief 中文：期望的列主序底层存储。 English: Expected column-major backing storage.
     */
    const std::vector<double> expected_column_major{0.0, 100.0, 1.0, 101.0, 2.0, 102.0};

    /**
     * @brief 中文：期望的带跨距底层存储。 English: Expected strided backing storage.
     */
    const std::vector<double> expected_strided{0.0, 1.0, 2.0, 0.0, 0.0, 100.0, 101.0, 102.0, 0.0, 0.0};

    /**
     * @brief 中文：期望的逐行分配第一行存储。 English: Expected first-row storage for the jagged layout.
     */
    const std::vector<double> expected_jagged_row0{0.0, 1.0, 2.0};

    /**
     * @brief 中文：期望的逐行分配第二行存储。 English: Expected second-row storage for the jagged layout.
     */
    const std::vector<double> expected_jagged_row1{100.0, 101.0, 102.0};

    /**
     * @brief 中文：期望的分块底层存储。 English: Expected blocked backing storage.
     */
    ...
    /**
     * @brief 中文：期望的 Morton 底层存储。 English: Expected Morton backing storage.
     */
    ...
    /**
     * @brief 中文：非法 stride 自动正规化后的测试矩阵。 English: Test matrix after automatic normalization of an invalid stride.
     */
    mpilab::domain::StridedRowMajorMatrix<> normalized(3, 2, 1);

    /**
     * @brief 中文：非法块大小自动正规化后的测试矩阵。 English: Test matrix after automatic normalization of an invalid block size.
     */
